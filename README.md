...
- 메일 제목은 `[KRIBB meal] YYYY/MM/DD 식단 미갱신`. 본문에 확인 시각, 가능한 원인, 조치(서버에서 `node kribb-meal-bot.mjs` 수동 실행, 로그 확인)가 들어 있다.
- 메일 발송이 실패하면 다음 분에 다시 시도한다.

반영 방법:

1. 코드를 Apps Script에 붙여넣고 저장한 뒤 **배포 관리 → 연필 아이콘 → 버전 "새 버전" → 배포**. `setup` 은 재실행하지 말 것.
2. 처음 한 번은 메일 발송(MailApp) 권한 승인이 필요하다. 편집기 상단 함수 목록에서 `watchdogCheck` 를 골라 **실행**하고 권한 승인 창에서 허용한다. 승인 전에는 트리거에서 메일 발송이 실패한다.

수신 주소는 기본값이 스크립트 소유자 계정이다. 다른 주소로 받으려면 `apps-script-code.js` 상단의 `WATCHDOG_EMAIL` 상수에 주소를 넣고 다시 배포한다. 알림 시작 시각은 `WATCHDOG_H`, `WATCHDOG_M` 상수로 바꾼다.
