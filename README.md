...
`HOLIDAY_RANGES`에 없는 갑작스러운 휴무(식당 사정, 행사 등)는 텔레그램 명령으로 등록한다. 등록한 날에는 watchdog 메일이 나가지 않는다. 식단 알림 동작은 그대로이며 휴무일에는 식단 데이터가 없어 알림도 나가지 않는다.

- **프로젝트 설정 → 스크립트 속성**에 `ADMIN_CHAT_ID` = 관리자 본인의 텔레그램 chat id를 추가한다. 기존 `.env`의 `TELEGRAM_CHAT_ID` 값과 같을 수 있다. 비워 두면 아래 명령은 모두 무시된다(모르는 명령처럼 응답 없음).
- 관리자 chat에서만 동작하며 `/help`에는 나오지 않는다.

| 명령 | 동작 |
|------|------|
| `/skip YYYY-MM-DD [YYYY-MM-DD]` | 하루 또는 양끝 포함 범위를 휴무일로 등록 (최대 31일) |
| `/unskip YYYY-MM-DD` | 해당 날짜 등록 해제 |
| `/skips` | 오늘 이후 등록된 휴무일 목록 |

목록은 스크립트 속성 `skipDates`에 저장되고 지난 날짜는 등록·조회 때 정리된다.
