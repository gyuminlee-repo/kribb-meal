...
집중휴가처럼 구내식당이 쉬는 기간에는 `/lunch`, `/dinner`, `/meal`, `/test` 응답을 안내 문구로 대체할 수 있다. Apps Script **스크립트 속성**만 바꾸면 되고, 코드 재배포나 크롤러(우분투 서버) 변경은 필요 없다.

1. [script.google.com](https://script.google.com) → 프로젝트 → 톱니바퀴(프로젝트 설정) → 스크립트 속성 → 속성 추가
2. 속성 이름 `holiday`, 값에 아래 JSON 입력 (날짜는 `YYYY/MM/DD`, from·to 모두 포함, KST 기준)

```json
{"from":"2026/07/27","to":"2026/07/31","msg":"집중휴가기간(7/27~7/31)이라 구내식당 식단 운영이 없습니다.\n다음 주 월요일부터 정상 안내됩니다."}
```

3. 저장

기간이 지나면 자동으로 평상시 동작으로 돌아간다. 즉시 해제하려면 `holiday` 속성을 삭제한다. 값이 없거나 JSON이 깨져 있으면 무시하고 평상시대로 동작한다.

`msg`를 생략하면 기본 문구(`집중휴가기간이라 이번 주 식단 운영이 없습니다.`)가 나간다. 자동 알림(09:30)은 휴가 기간에 크롤링 데이터 자체가 없어 전송되지 않는다.
